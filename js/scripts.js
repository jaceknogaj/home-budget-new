...
editForm.addEventListener("submit", (e) => {
  ...
  const newAmount = parseFloat(editAmount.value).toFixed(2);

  if (newName && !isNaN(newAmount)) {
    if (editingType === "income") {
      incomeData[editingIndex].name = newName;
      incomeData[editingIndex].amount = newAmount;
    } else {
      expenseData[editingIndex].name = newName;
      expenseData[editingIndex].amount = newAmount;
    }
...
function updateBudgetList() {
  ...
  for (let i = 0; i < incomeData.length; i++) {
    ...
    divTextRight.textContent = item.amount.toFixed(2) + " PLN";
...
function openEditModal(index, type) {
  ...
  editSaveBtn.onclick = () => {
    ...
    if (newName && !isNaN(newAmount)) {
      if (type === "expense") {
        expenseData[index].name = newName;
        expenseData[index].amount = newAmount;
      } else {
        incomeData[index].name = newName;
